...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const apiUrl = `${API_HOST}api/register/users/`;
      ...
      if (response.ok) {
        ...
        if (errorData.email) {
          setAlert({ type: 'error', message: `Email is already in use: ${errorData.email}` });
        } else if (errorData.phone) {
          setAlert({ type: 'error', message: `Phone number is already in use: ${errorData.phone}` });
